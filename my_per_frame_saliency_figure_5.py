...
def compress_saliency(video_input, high_quality_qp, low_quality_qp, smooth = 1, img_w = 1280, img_h = 720):
    ...
    fid = int(video_input.split("_")[-1])

    if not os.path.exists(working_dir_path + f"{video_input}_qp{low_quality_qp}_local_codec/") or \
       not len(os.listdir(working_dir_path + f"{video_input}_qp{low_quality_qp}_local_codec/")):
    ...
    saliency_mask_mb_tiled = None
    if smooth == 1 or (fid - 1) % smooth == 0:

        # for fid in range(len(saliency_original_frames)):

        # video_slice_gt = saliency_original_frames[0]['image'].unsqueeze(0)
        # video_slice_lq = saliency_lq_frames[0]['image'].unsqueeze(0)
        ...
        # save mask
        with open(f"masks/{video_input}_saliency_per_frame_{high_quality_qp}_{low_quality_qp}_fig4.pt", "wb") as f:
        ...
        mask_fid = int((fid - 1) // smooth * smooth + 1)
        vid_name = "_".join(video_input.split("_")[:-1]) + "_" + str(mask_fid)

        with open(f"masks/{vid_name}_saliency_per_frame_{high_quality_qp}_{low_quality_qp}_fig4.pt", "rb") as f:
            saliency_mask_mb_tiled = pickle.load(f)
...
def my_inference(video_input, high_quality_qp, low_quality_qp, smooth, stats_file_name):
    ...
    decode_video(working_dir_path + f"{vid_name}_saliency_per_frame_lq_{high_quality_qp}_{low_quality_qp}.mp4",
                 working_dir_path + f"{vid_name}_saliency_per_frame_lq_{high_quality_qp}_{low_quality_qp}_decoded",)
    saliency_low_quality_frames = Video_Dataset(working_dir_path + f"{vid_name}_saliency_per_frame_lq_{high_quality_qp}_{low_quality_qp}_decoded/")
    ...
    video_slice_gt = gt_frames[0]['image'].unsqueeze(0)
    gt_result = infer(video_slice_gt, app)

    # non-smoothing
    saliency_high_quality_frame = saliency_high_quality_frames[0]['image'].unsqueeze(0)
    saliency_low_quality_frame = saliency_low_quality_frames[0]['image'].unsqueeze(0)
    ...
    # with open(f"stats_files/tradeoff_files/tf2_300_frames_saliency_per_frame_figure_4_refixed_more_more_frames_1102", "a+") as f:
    with open(stats_file_name, "a+") as f:
        if not isinstance(loss_saliency, float):
            f.write(f"{video_input},{smooth},{high_quality_qp},{low_quality_qp},{abs(loss_saliency).item()},{saliency_fs}\n")
        else:
            f.write(f"{video_input},{smooth},{high_quality_qp},{low_quality_qp},{abs(loss_saliency)},{saliency_fs}\n")
...
##### main function CUDA_VISIBLE_DEVICES=1 python my_per_frame_saliency_figure_4.py
# video_list = [f"videos/trafficcam_2_{i}" for i in range(1,1801,10)]
# video_list = [f"videos/trafficcam_2_{i}" for i in range(1501,1601,1)]
...
for frame_index in range(len(video_list)):
    ...
    for smooth in smooths:
        for i in range(len(hq_qps)):
            ...
            stats_file_name = f"stats_files_cr/{vid_name}_300_frames_saliency_figure_4_{hq_qp}_{lq_qp}_{smooth}_cr"
            # stats_file_name = "stats_files/tmp"
            
            # compression
            compress_saliency(video, hq_qp, lq_qp, smooth, resolution[0], resolution[1])

            # inference
            my_inference(video, hq_qp, lq_qp, smooth, stats_file_name)

# remove all the masks
for frame_index in range(len(video_list)):
    video = video_list[frame_index]
    if os.path.exists(f"masks/{video}_saliency_per_frame_2_32_fig4.pt"):
        os.remove(f"masks/{video}_saliency_per_frame_2_32_fig4.pt")
...
#             # compression
#             compress_saliency(video, hq_qp, lq_qp, smooth)

#             # inference
#             my_inference(video, hq_qp, lq_qp, smooth)
