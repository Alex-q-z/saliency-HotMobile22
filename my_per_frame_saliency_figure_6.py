...
def compress_saliency(video_input, high_quality_qp, low_quality_qp, smooth = 1, img_w = 1280, img_h = 720):
    ...
    if smooth == 1 or (fid - 1) % smooth == 0:
        ...
        # save mask
        with open(f"masks/{video_input}_saliency_per_frame_{high_quality_qp}_{low_quality_qp}_fig5.pt", "wb") as f:
        ...
        with open(f"masks/{vid_name}_saliency_per_frame_{high_quality_qp}_{low_quality_qp}_fig5.pt", "rb") as f:
...
def my_inference(video_input, high_quality_qp, low_quality_qp, smooth, stats_file_name):
    ...
    saliency_low_quality_frames_fs = os.path.getsize(working_dir_path + f"{vid_name}_saliency_per_frame_lq_{high_quality_qp}_{low_quality_qp}.mp4")
    ...
    # with open(f"stats_files/tradeoff_files/tf2_300_frames_saliency_per_frame_figure_5_fixed_new_1102", "a+") as f:
...
for frame_index in range(len(video_list)):
    ...
    for smooth in smooths:
        for i in range(len(hq_qps)):
            ...
            stats_file_name = f"stats_files_cr/{vid_name}_300_frames_saliency_figure_5_{hq_qp}_{lq_qp}_{smooth}_cr"
