...
class FasterRCNN_ResNet50_FPN(DNN):
    ...
    def region_proposal(self, video):
        ...
        proposals, scores = self.model.rpn(images, features, targets, return_scores=True)
        # import pdb; pdb.set_trace()

        ret = Instances(
            image_size=(720, 1280),
            proposal_boxes=Boxes(proposals[0].detach().cpu().numpy()),
            objectness_logits=scores[0].detach().cpu().numpy()
        )
